...
def list_interactions(user: CurrentUser, session: Database) -> list[JobInteraction]:
    return list(
        session.scalars(
...
def update_interaction(
    job_id: uuid.UUID, payload: InteractionUpdate, user: CurrentUser, session: Database
) -> object:
...
def record_view(job_id: uuid.UUID, user: CurrentUser, session: Database) -> object:
    _owned_job(session, user.id, job_id)
    interaction = _interaction(session, user.id, job_id)
    now = datetime.now(UTC)
...
def create_share(
    job_id: uuid.UUID, payload: ShareCreate, user: CurrentUser, session: Database
) -> ShareResponse:
    _owned_job(session, user.id, job_id)
...
def revoke_share(share_id: uuid.UUID, user: CurrentUser, session: Database) -> Response:
...
def private_shared_job(token: str, session: Database) -> PublicJobResponse:
    share = session.scalar(
...
def similar_jobs(job_id: uuid.UUID, user: CurrentUser, session: Database) -> list[JobResponse]:
