...
class JobPersister:
    ...
    def persist(
        ...
        if canonical_job:
            ...
            canonical_job.sources.append(self._source_record(candidate, seen_at))
        ...
        job.sources.append(self._source_record(candidate, seen_at))
        session.add(job)
        return PersistenceOutcome.CREATED

    @staticmethod
    def _source_record(candidate: NormalizedJob, seen_at: datetime) -> JobSource:
        return JobSource(
