...
function SourceCard({ value, changed, removed }: { value: AdminSource; changed: (value: AdminSource) => void; removed: () => void }) {
  ...
  return <article className={`admin-source-card ${value.enabled ? "active" : ""}`}>
    ...
    <div className="admin-source-actions">
      ...
      <button disabled={Boolean(pending)} onClick={() => void action("Preview", async () => { const result = await api.previewAdminSource(value.id); setPreview(result); changed({ ...value, last_validated_at: new Date().toISOString() }); })}><Eye size={15} />Preview</button>
