...
import { AlertTriangle, ArrowRight, BellRing, CheckCircle2, Filter } from "lucide-react";
...
export function DashboardView() {
  ...
  return <div className="app-page"><PageHeader eyebrow="Overview" title="Your internship signal" copy="A quick read on what Sprintern has found and what needs your attention."/>
    {data.sourceHealth.state === "stale" && <div className="source-warning" role="status"><AlertTriangle size={19}/><span><strong>Job checks have not refreshed recently</strong><small>The last successful source update was {freshness(data.sourceHealth)}. Existing jobs remain available, but new listings may be missing until the scheduler runs again.</small></span></div>}
...
function Metric({label,value}:{label:string;value:string|number}){return <article className="metric-card"><div><small>{label}</small><strong>{value}</strong></div></article>}
