...
export function DashboardView() {
  ...
  return <div className="app-page"><PageHeader eyebrow="Overview" title="Your internship signal" copy="A quick read on what Sprintern has found and what needs your attention."/>
    {data.sourceHealth.state === "stale" && <div className="source-warning" role="status"><AlertTriangle size={19}/><span><strong>Source updates are delayed</strong><small>Existing matches remain available while Sprintern catches up.</small></span></div>}
    ...
    <section className="metric-grid"><Metric icon={<BellRing/>} label="Matches" value={data.analytics.matched_count}/><Metric icon={<Send/>} label="Applied" value={data.analytics.applied_count}/><Metric icon={<Clock3/>} label="Avg. time to apply" value={formatDuration(data.analytics.average_seconds_to_apply)}/><Metric icon={<Radio/>} label="Sources updated" value={freshness(data.sourceHealth)}/></section>
