...
function TermPicker({ values, onChange }: { values: string[]; onChange: (values: string[]) => void }) { return <div className="field term-field"><span className="field-label">Internship terms</span><div className="term-options" role="group" aria-label="Internship terms"><button aria-pressed={!values.length} className={!values.length ? "active" : ""} onClick={() => onChange([])} type="button">Any term</button>{termOptions.map((term) => <button aria-pressed={values.includes(term)} className={values.includes(term) ? "active" : ""} key={term} onClick={() => onChange(values.includes(term) ? values.filter((value) => value !== term) : [...values, term])} type="button">{term}</button>)}</div></div>; }
