...
export function MatchesView() {
  ...
  const shown = useMemo(() => {
    const byStatus = tab === "all" ? items : items.filter((item) => item.status === tab);
    return showHidden
      ? byStatus.filter((item) => Boolean(interactions[item.job.id]?.hidden_at))
      : byStatus;
  }, [interactions, items, showHidden, tab]);

  function recordViewed(jobId: string) {
    if (viewedIds.has(jobId)) return;
    setViewedIds((current) => new Set(current).add(jobId));
    void api.recordView(jobId).catch(() => {
  ...
  async function showSimilar(item: JobMatch) {
    recordViewed(item.job.id);
    try {
      setSimilar({ owner: item.job, jobs: await api.similarJobs(item.job.id) });
  ...
  return <div className="app-page matches-page">
    ...
    <div className="discovery-toolbar">
      ...
      <select aria-label="Job collection" value={collection ?? ""} disabled={showHidden} onChange={(event) => setCollection((event.target.value || undefined) as Collection | undefined)}><option value="">All jobs</option>{collections.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
      <button className={`hidden-jobs-toggle ${showHidden ? "active" : ""}`} aria-pressed={showHidden} onClick={() => { setShowHidden((value) => !value); setCollection(undefined); }}><EyeOff size={16} />{showHidden ? "Back to feed" : "Hidden jobs"}</button>
    </div>
    ...
    {similar && <SimilarPanel owner={similar.owner} jobs={similar.jobs} close={() => setSimilar(null)} />}
...
function SimilarPanel({ owner, jobs, close }: { owner: Job; jobs: Job[]; close: () => void }) {
  const sameEmployer = jobs.filter((job) => job.company.trim().toLowerCase() === owner.company.trim().toLowerCase());
  const otherEmployers = jobs.filter((job) => !sameEmployer.includes(job));
  const group = (title: string, values: Job[]) => values.length ? <section>
    <h3>{title}</h3>
    <div>{values.map((job) => <a href={`/jobs/${job.id}`} target="_blank" rel="noopener noreferrer" key={job.id}><small>{job.company}</small><strong>{job.title}</strong><span>{job.location ?? "Location unknown"}</span></a>)}</div>
  </section> : null;
  return <section className="similar-panel">
    <div><span><strong>Similar to</strong> {owner.title}</span><button onClick={close}><X size={16} />Close</button></div>
    {jobs.length ? <div className="similar-groups">{group(`More roles at ${owner.company}`, sameEmployer)}{group("Similar jobs elsewhere", otherEmployers)}</div> : <p>No close alternatives are currently in your matches.</p>}
  </section>;
}
...
function deadlineLabel(value: string) { const days = Math.ceil((new Date(value).getTime() - Date.now()) / 86400000); return days < 0 ? "Deadline passed" : days === 0 ? "Apply soon · due today" : days <= 7 ? `Apply soon · ${days}d left` : `${days}d left`; }
