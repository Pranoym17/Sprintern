...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isSignUp) {
      ...
      if (passwordError) { setPending(false); setMessage(passwordError); return; }
      ...
      setPending(false);
      if (error) return setMessage(friendlyAuthError(error.message, "Your account could not be created. Please try again."));
      if (!data.session) return setMessage("Check your inbox to confirm your account, then sign in.");
    } else {
      const { error } = await supabase.auth.signInWithPassword({ email, password });
      if (error) { setPending(false); return setMessage(friendlyAuthError(error.message, "Sign-in failed. Please try again.")); }
    }
  ...
  return <form className="auth-form" onSubmit={submit} aria-busy={pending}>
    ...
    <div className="field"><div className="field-label-row"><label htmlFor="password">Password</label>{!isSignUp && <Link href="/forgot-password">Forgot password?</Link>}</div><input id="password" name="password" type="password" minLength={8} autoComplete={isSignUp ? "new-password" : "current-password"} required />{isSignUp && <span className="field__help">{PASSWORD_REQUIREMENTS}</span>}</div>
