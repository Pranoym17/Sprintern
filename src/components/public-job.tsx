...
export function PublicJob({ job, expiresAt }: { job: Job; expiresAt?: string | null }) {
  const applyUrl = job.sources.find((source) => source.apply_url)?.apply_url;
  return <main className="public-job-page">
    ...
    <article>
      ...
      {job.title_incomplete && <p className="quality-warning">The source provided an incomplete title.</p>}
