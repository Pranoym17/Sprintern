...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  return <form className="auth-form" onSubmit={submit} aria-busy={pending}>
    ...
    <button className="button button--primary button--full" disabled={pending}>{pending ? <LoaderCircle className="spin" size={18} /> : null}{isSignUp ? "Create account" : "Sign in"}<ArrowRight size={18} /></button>
