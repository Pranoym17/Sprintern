...
export interface Profile { id:string; email:string|null; timezone:string; notification_cadence:NotificationCadence; telegram_chat_id:string|null; email_notifications_enabled:boolean; email_notifications_consent_at:string|null; email_suppressed_at:string|null; email_suppression_reason:string|null; telegram_notifications_enabled:boolean; quiet_hours_start:string|null; quiet_hours_end:string|null; weekend_pause:boolean; max_alerts_per_day:number; priority_only_instant:boolean; notification_consents:Record<string,boolean>; created_at:string; updated_at:string; }
export interface ProfileUpdate { timezone?:string; notification_cadence?:NotificationCadence; email_notifications_enabled?:boolean; telegram_notifications_enabled?:boolean; quiet_hours_start?:string|null; quiet_hours_end?:string|null; weekend_pause?:boolean; max_alerts_per_day?:number; priority_only_instant?:boolean; notification_consents?:Record<string,boolean>; }
...
export interface DeliverySummary { channel:DeliveryChannel; status:DeliveryStatus; cadence:NotificationCadence; priority:"normal"|"high"; queued_reason:string|null; sent_at:string|null; }
...
export interface FilterNotification { filter_id:string; email_enabled:boolean|null; telegram_enabled:boolean|null; cadence:NotificationCadence|null; priority:"normal"|"high"; uses_profile_defaults:boolean; }
export interface DeliveryQueue { pending:number; delayed_by_quiet_hours:number; delayed_by_weekend:number; delayed_by_daily_cap:number; failed:number; suppressed:number; }
...
export interface AdminSource { id:string; source_key:string; owner:string; repository:string; branch:string|null; path:string; enabled:boolean; poll_minutes:number; jitter_seconds:number; default_term:string|null; parser_schema:string; parser_version:string; last_validated_at:string|null; last_succeeded_at:string|null; last_failed_at:string|null; consecutive_failures:number; last_error:string|null; created_at:string; updated_at:string; }
export interface AdminSourceInput { owner:string; repository:string; branch:string|null; path:string; poll_minutes:number; jitter_seconds:number; default_term:string|null; parser_schema:string; parser_version:string; }
export interface SourcePreview { rows_fetched:number; accepted:number; rejected:number; duplicate_candidates:number; sample_normalized_output:{company:string;title:string;location:string|null;term:string|null;application_url:string;application_domain:string|null;canonical_fingerprint:string}[]; detected_table_schema:string; missing_columns:string[]; rejected_rows:string[]; suspicious_truncated_values:string[]; inferred_terms:{term:string;count:number}[]; application_domains:string[]; }
export interface AdminSourceRun { id:string;status:string;started_at:string;finished_at:string|null;fetched_count:number;accepted_count:number;rejected_count:number;created_count:number;updated_count:number;duplicate_count:number;error:string|null; }
export interface SourceAudit { id:string;source_configuration_id:string|null;administrator_id:string;action:string;details:Record<string,unknown>;request_id:string|null;created_at:string; }
