...
async def test_authenticated_mvp_workflow_is_idempotent(
    ...
    # Production performs this stage through the durable worker.
    matching_service.match_all(db_session)
    db_session.commit()
    ...
    assert await dispatcher.dispatch_due(now=now + timedelta(days=2), limit=100) == 0
